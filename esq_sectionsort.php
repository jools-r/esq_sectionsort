...
if (
safe_alter('txp_section', 'ADD `sectionsort` TINYINT NULL') &&
safe_alter('txp_category', 'ADD `categorysort` TINYINT NULL')
) {
...
if (
safe_alter('txp_section', 'MODIFY `sectionsort` TINYINT NULL') &&
safe_alter('txp_category', 'ADD `categorysort` TINYINT NULL')
) {
return '<strong>esq_sectionsort</strong> and DB updated OK.';
} else {
return array(
'<strong>esq_sectionsort</strong> updated, but DB update failed. Try adjusting a section\'s order, then disable and enable the plugin again.',
E_ERROR
);
}
}
